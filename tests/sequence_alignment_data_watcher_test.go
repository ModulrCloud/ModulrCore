...
func simulateSequenceAlignmentThreadForTest(t *testing.T, metadata *structures.ExecutionThreadMetadataHandler, epochHandler structures.EpochDataHandler, fetcher testAnchorBlockFetcher, maxSteps int) {
	...
	for step := 0; step < maxSteps; step++ {
		...
		currentExec := metadata.SequenceAlignmentData.CurrentLeaderToExecBlocksFrom
		...
		if currentExec < target.Index {
			...
			metadata.SequenceAlignmentData.CurrentLeaderToExecBlocksFrom = currentExec + 1
			continue
		}
		...
		if currentExec == target.Index {
			...
			metadata.SequenceAlignmentData.CurrentLeaderToExecBlocksFrom = 0
			continue
		}
